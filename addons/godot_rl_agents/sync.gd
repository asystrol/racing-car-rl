...
#func _ready():
	#await get_parent().process_frame
...
# Called when the node enters the scene tree for the first time.
func _ready():
	# Wait until this node is definitely inside the tree structure
	if not is_inside_tree():
		await tree_entered
		
	# Safely wait one frame using the active scene tree object
...
func _initialize_training_agents():
	if agents_training.size() > 0:
		...
		if connected:
			...
			push_warning("Couldn't connect to Python server.")
	else:
		# ADD THIS CRITICAL LOG LINE TO DEBUG:
		print("CRITICAL: No training agents detected in the 'AGENT' group!")
...
func _set_agent_mode(agent: Node):
	print("FORCING AGENT INTO TRAINING MODE")
	# This bypasses the inspector and forces the enum to 2 (Training)
...
func _get_agents():
	...
	print("Total agents found in AGENT group: ", all_agents.size())
	
	for agent in all_agents:
		print("-> Found Agent Path: ", agent.get_path())
		print("-> Initial Control Mode (Enum Int): ", agent.control_mode)
		
		_set_agent_mode(agent)
		print("-> Mode after Sync override: ", agent.control_mode)
		
		if agent.control_mode == agent.ControlModes.TRAINING:
			agents_training.append(agent)
			print("-> SUCCESS: Added to agents_training array.")
		elif agent.control_mode == agent.ControlModes.ONNX_INFERENCE:
			agents_inference.append(agent)
			print("-> FAILED: Added to agents_inference instead.")
		elif agent.control_mode == agent.ControlModes.HUMAN:
			agents_heuristic.append(agent)
			print("-> FAILED: Added to agents_heuristic (Human) instead.")
